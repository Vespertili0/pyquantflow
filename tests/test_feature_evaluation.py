...
class TestFeatureEvaluation(unittest.TestCase):
    ...
    def test_feature_evaluator_profiling(self):
        ...
        self.assertIn("adf_p_value", profiles.columns)
        self.assertIn("acf1", profiles.columns)
        self.assertIn("spectral_entropy", profiles.columns)
        self.assertEqual(len(profiles), 2)  # Two features
    ...
    def test_feature_evaluator_clustering_and_mda(self):
        ...
        clusters = evaluator.cluster_entities(df, evaluator.features)
        ...
        # First, transform features to avoid NaNs crashing sklearn
        ...
        self.assertIn("MDA", results)
        self.assertIn("SFI", results)
        self.assertTrue(len(results["MDA"]) > 0)
