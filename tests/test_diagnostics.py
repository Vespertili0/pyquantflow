...
class TestFoundation(unittest.TestCase):
    ...
    def test_return_types(self):
        ...
        res4 = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=2.0, sl_mult=2.0, horizon=5
        )
...
class TestCUSUMEvents(unittest.TestCase):
    ...
    def test_marker_timestamps_match_events(self):
        df, events = make_cusum_events()
        res = plot_cusum_events(df, events=events)
        marker_trace = [t for t in res.figure.data if isinstance(t, go.Scatter) and t.mode == "markers"][0]
...
class TestBarrierTrajectories(unittest.TestCase):
    def test_n_trajectory_traces(self):
        ...
        res = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=1.0, sl_mult=1.0, horizon=5, n_events=n_events
        )
        ...
        path_traces = [t for t in res.figure.data if getattr(t, "xaxis", None) == "x" and getattr(t, "yaxis", None) == "y" and t.name is None]
    ...
    def test_traces_within_horizon(self):
        ...
        res = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=1.0, sl_mult=1.0, horizon=horizon, n_events=5
        )
    ...
    def test_pct_sums_to_one(self):
        ...
        res = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=1.0, sl_mult=1.0, horizon=5
        )
        pct_sum = res.metadata["pct_tp"] + res.metadata["pct_sl"] + res.metadata["pct_timeout"]
    ...
    def test_trace_colours_match_labels(self):
        ...
        res = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=1.0, sl_mult=1.0, horizon=5, n_events=1
        )
    ...
    def test_n_events_cap(self):
        ...
        res = plot_barrier_trajectories(
            barrier_df, event_timestamps=barrier_df.index,
            pt_mult=1.0, sl_mult=1.0, horizon=5, n_events=500
        )
