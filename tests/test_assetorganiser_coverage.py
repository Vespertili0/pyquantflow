...
class TestAssetOrganiserCoverage(unittest.TestCase):
    def setUp(self):
        ...
        if len(self.data_map) < 2:
            ...
            df_a = pd.DataFrame({"Close": 100.0 + np.cumsum(np.random.normal(0, 1.0, 100)),
                                 "feature1": np.random.randn(100),
                                 "target": np.random.randint(0, 2, 100)}, index=dates)
            df_a["returns"] = df_a["Close"].pct_change()
            df_a.index.name = "datetime"

            df_b = pd.DataFrame({"Close": 100.0 + np.cumsum(np.random.normal(0, 1.0, 100)),
                                 "feature1": np.random.randn(100),
                                 "target": np.random.randint(0, 2, 100)}, index=dates)
    ...
    def test_downsample_to_events_type_error(self):
        organiser = AssetOrganiser(data_map=self.data_map, cutoff_date=self.cutoff_date, target_features=["target"])
    ...
    def test_downsample_cusum_training_data_missing(self):
        ...
        with self.assertRaises(ValueError):
            organiser.downsample_to_cusum_events(target_events_train=5, filter_col="returns")

    def test_downsample_cusum_ticker_missing_train(self):
        # Create a ticker that is only present in test data
        dates = pd.date_range(pd.to_datetime(self.cutoff_date) + pd.Timedelta(days=1), periods=10)
        df_c = pd.DataFrame({"Close": 100.0, "feature1": 0.0, "target": 0, "returns": 0.0}, index=dates)
        df_c.index.name = "datetime"
        data_map = self.data_map.copy()
        data_map["CCC"] = df_c

        organiser = AssetOrganiser(data_map=data_map, cutoff_date=self.cutoff_date, target_features=["target"])
        organiser.prepare_multi_asset_frame()
        
        # Ticker CCC is not in train data, should fallback to alpha_min
        alphas = organiser.downsample_to_cusum_events(target_events_train=5, filter_col="returns", alpha_min=0.5)
    ...
    def test_downsample_cusum_target_events_dict_key_error(self):
        ...
        with self.assertRaises(KeyError):
            organiser.downsample_to_cusum_events(target_events_train={self.ticker_1: 5}, filter_col="returns")
    ...
    def test_downsample_cusum_missing_vol_col(self):
        ...
        alphas = organiser.downsample_to_cusum_events(target_events_train=5, filter_col="returns", vol_col="missing_vol")
    ...
    def test_add_model_predictions(self):
        ...
        organiser.add_model_predictions(model, features=["feature1"], prefix="secondary", filter_prediction=1)
    ...
    def test_replace_features(self):
        ...
        org_uninit = AssetOrganiser(data_map=self.data_map, cutoff_date=self.cutoff_date, target_features=["target"])
