...
class DatabaseManager:
    ...
    _SQL_SELECT_TICKER = (
        "SELECT id, last_updated, interval FROM tickers WHERE ticker = ?"
    )
    _SQL_SELECT_MAX_DATETIME = (
    ...
    def add_ticker(
        ...
        logger.info(
            f"Fetching initial data for {ticker} from {start} (interval={interval})..."
        )
    ...
    def _update_ticker_internal(self, ticker, ticker_id, interval, commit=True):
        ...
        if not last_date_str:
            logger.warning(
                f"No previous data found for {ticker}. Attempting fresh fetch."
            )
            ...
            if not df.empty:
                self._insert_price_data(ticker_id, df)
                cursor.execute(
                    self._SQL_UPDATE_TICKER_TIME, (datetime.now(), ticker_id)
                )
                if commit:
                    self.conn.commit()
    ...
    def _insert_price_data(self, ticker_id, df):
        ...
        dt_col = (
            ...
            else ("Date" if "Date" in df_reset.columns else df_reset.columns[0])
        )
